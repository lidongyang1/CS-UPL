...
model.load_weights('./model/CS-GCN-LSTM.h5')
predict = model.predict([testX,testX,testX_top_1,testX_top_2,testX_top_3])
np.save('Results/CS-GCN-LSTM/User=30_FileNum=400_predict.npy', predict)
np.save('Results/CS-GCN-LSTM/User=30_FileNum=400_test.npy', testY)
predict = np.array(getOneDimension(predict))
test = np.array(getOneDimension(testY))
