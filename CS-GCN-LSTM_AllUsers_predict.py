...
def getMostNSimilarityUEs(topN,Similarity):
    topN_index = []
    tmp = np.sort(Similarity)
    MostValues = []
    for i in range(topN):
        MostValues.append(tmp[len(Similarity)-i-1])
    for i in range(topN):
        for j in range(len(Similarity)):
            if MostValues[i] == Similarity[j]:
                topN_index.append(j)
    return topN_index
...
testX_allUser = []
testY_allUser = []
trainX_top_1_allUser = []
trainX_top_2_allUser = []
trainX_top_3_allUser = []
testX_top_1_allUser = []
testX_top_2_allUser = []
testX_top_3_allUser = []

for userId in range(userNum):
    view_counts = np.load('./YouTube_NewDATA/data/userData/users=30_FileNum=400/User_'+str(userId+1)+'_Contents.npy')
    ...
    trainX_allUser.extend(trainX)
    trainY_allUser.extend(trainY)
    testX_allUser.extend(testX)
    testY_allUser.extend(testY)
    print('userId',userId)
    SocialRelationship = np.load('./YouTube_NewDATA/data/userData/users=30_FileNum=400/socialRelationship.npy')
    SocialSimilarityUsers = np.delete(getMostNSimilarityUEs(top_k+1,SocialRelationship[userId]), 0, axis=0)
    ...
    trainX_top_1_allUser.extend(trainX_top_1)
    trainX_top_2_allUser.extend(trainX_top_2)
    trainX_top_3_allUser.extend(trainX_top_3)
    testX_top_1_allUser.extend(testX_top_1)
    testX_top_2_allUser.extend(testX_top_2)
    testX_top_3_allUser.extend(testX_top_3)

trainX_allUser = np.array(trainX_allUser)
trainY_allUser = np.array(trainY_allUser)
testX_allUser = np.array(testX_allUser)
testY_allUser = np.array(testY_allUser)
trainX_top_1_allUser = np.array(trainX_top_1_allUser)
trainX_top_2_allUser = np.array(trainX_top_2_allUser)
trainX_top_3_allUser = np.array(trainX_top_3_allUser)
testX_top_1_allUser = np.array(testX_top_1_allUser)
testX_top_2_allUser = np.array(testX_top_2_allUser)
testX_top_3_allUser = np.array(testX_top_3_allUser)
print(trainX_allUser.shape)
...
model.load_weights('./model/CS-GCN-LSTM_allUsers.h5')
predict = model.predict([testX_allUser,testX_allUser,testX_top_1_allUser,testX_top_2_allUser,testX_top_3_allUser])
np.save('Results/CS-GCN-LSTM_AllUsers/User=30_FileNum=400_predict.npy', predict)
np.save('Results/CS-GCN-LSTM_AllUsers/User=30_FileNum=400_test.npy', testY_allUser)
predict = np.array(getOneDimension(predict))
test = np.array(getOneDimension(testY_allUser))

print(mean_absolute_error(predict,test))
print(np.sqrt(mean_squared_error(predict,test)))
print(r2_score(predict,test))
